...
function sArena_CastBar_OnEvent(self, event, ...)
    if db.profile.castBarEnabled == false then
        self:Hide()
        return
    end
...
function sArenaFrameMixin:UpdateClassIcon()
    ...
    if db.profile.classSpecEnabled == false then
        self.ClassIcon:Hide()
        if self.ClassIconCooldown then self.ClassIconCooldown:Clear() end
        if self.SpecIcon then self.SpecIcon:Hide() end
        if self.SpecBorderOverlay then self.SpecBorderOverlay:Hide() end
        return
    end
    self.ClassIcon:Show()
    ...
    if (texture == "class") then
        local coords = classIcons[self.class]
        if coords then
            -- Base WotLK class: crop the shared class-icon atlas as before
            self.ClassIcon:SetTexture(iconPath, true)
            self.ClassIcon:SetTexCoord(unpack(coords))
            return
        end
        -- self.class isn't one of the 10 keys classIcons knows (this is the case for
        -- Conquest of Azeroth characters) -- previously this called unpack(nil) here,
